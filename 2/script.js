...
function dataReportStatus(data) {
    ...
    for (let i = 0; i < data.tracks.items.length; i++) {
        if((data.tracks.items[i].album.name.toUpperCase()).includes(text.toUpperCase())){
        ...
        }else if((data.tracks.items[i].album.artists[0].name.toUpperCase()).includes(text.toUpperCase())){
            colToPush.innerHTML += `<div class="col-4 mt-5">
        <div class="card" style="width: 18rem;">
            <img class="card-img-top" src="${data.tracks.items[i].album.images[0].url}" alt="Card image cap">
            <div class="card-body">
